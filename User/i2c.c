...
uint8_t i2c2_check_flag(uint32_t flag){
    ...
    //return i2c_intr_error||I2C_GetFlagStatus( I2C2, flag ) == RESET;
}
uint8_t i2c2_check_event(uint32_t event){
    return i2c2_check_error()||I2C_CheckEvent(I2C2, event);
    //return i2c_intr_error||I2C_CheckEvent(I2C2, event);
}
/* Global define */
/*********************************************************************
 * @fn      IIC_Init
...
void i2c_hardware_init(u32 baudrate, u16 address, u8 allow_reset)
{
    ...
    if(execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_flag),(void*)I2C_FLAG_BUSY,IMU_I2C_TIMEOUT_US)){
...
void i2c2_reset()
{
    ...
    //I2C_Cmd(I2C2, ENABLE);
    //res&=!execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_flag),(void*)I2C_FLAG_BUSY,IMU_I2C_TIMEOUT_US);
    if(res|i2c_intr_error){
        ...
        //Delay_Us(10);
        printf("i2c sleep %d\r\n",execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_flag),(void*)I2C_FLAG_BUSY,IMU_I2C_TIMEOUT_US));
    }else {
...
uint8_t i2c_read_start(uint8_t addr,uint8_t ack_mode)
{
    ...
    if(!i2c_status)return 1;
        ...
        do{
            ...
            res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_EVENT_MASTER_MODE_SELECT,IMU_I2C_TIMEOUT_US*100);
            if(res|i2c_intr_error){
                i2c_error_code=2;
                break;
            }
            ...
            res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED,IMU_I2C_TIMEOUT_US);
            ...
            //res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_EVENT_MASTER_BYTE_TRANSMITTED,IMU_I2C_TIMEOUT_US);
            if(res|i2c_intr_error){
                i2c_error_code=4;
                break;
            }
            ...
            I2C_GenerateSTART( I2C2, ENABLE );
            res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_EVENT_MASTER_MODE_SELECT,IMU_I2C_TIMEOUT_US);
            if(res|i2c_intr_error){
                i2c_error_code=5;
                break;
            }
            I2C_Send7bitAddress(I2C2, IMU_ADDR, I2C_Direction_Receiver);
            res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED,IMU_I2C_TIMEOUT_US);
            if(res|i2c_intr_error){
                i2c_error_code=6;
                break;
            }
...
uint8_t i2c_read_continuous(uint8_t addr,uint8_t* buf,uint8_t len)
{
    ...
    while(len--){
        //res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_FLAG_RXNE,IMU_I2C_TIMEOUT_US);
        res=execute_blocking_us(IMU_I2C_EXECUTE_FUNC(i2c2_check_event),(void*)I2C_EVENT_MASTER_BYTE_RECEIVED,IMU_I2C_TIMEOUT_US);
        if(res|i2c_intr_error){
...
uint8_t i2c_read_byte(uint8_t addr,uint8_t *ret)
{
    uint8_t res=0;
    do{
